    ...
    <>
      <div className={styles.post_page_container} >
        <div className={styles.post_img_container}>
          {posts.map(({ url, userIndex, userUrl, name }, index) => <div className={styles.posts_container} key={index}><div className={styles.user_container}>< img className={styles.user_img} src={userUrl} alt="user"/><h3 className={styles.user_name}>{name}</h3></div><img className={styles.posts_img} src={url} alt="post" />{products[index].comments.map((el,index) => <div className={styles.comments_container} key={index}><h3 className={styles.comment_user_name}>{users[el.userIndex].name}</h3><h3 className={styles.comment_user_text}>{el.text}</h3></div>)}<span className={styles.posts_span}>Likes:{products[index].likes}</span><svg onClick={() => {
